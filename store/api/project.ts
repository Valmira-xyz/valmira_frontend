...
import type {
  ...
  GlobalMetrics,
} from '@/services/projectService'; // Adjust path if necessary
import type { Project, ProjectStatistics } from '@/types'; // Adjust path if necessary
...
export const projectApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    >({
      query: ({ pageIndex = 0, maxPageCount = 10 }) =>
        `projects/public?pageIndex=${pageIndex}&maxPageCount=${maxPageCount}`,
      providesTags: (result) =>
        result
          ? [
              ...result.map(({ _id }) => ({
    ...
    getVolumeData: builder.query<any, string>({
      // Replace 'any' with the actual type for volume data
      query: (projectId) => `projects/${projectId}/volume`, // Adjust path as needed
      providesTags: (result, error, projectId) => [
        { type: 'VolumeData', id: projectId },
      ],
    }),
    getProjectStats: builder.query<
      ProjectStatistics,
      { projectId: string; timeRange?: { start: string; end: string } }
    >({
    ...
    >({
      query: ({ projectId, limit }) =>
        `projects/${projectId}/activity${limit ? `?limit=${limit}` : ''}`, // Adjust path as needed
      providesTags: (result, error, { projectId }) => [
        { type: 'Activity', id: projectId },
      ],
    }),
    getBotPerformanceHistory: builder.query<
      BotPerformanceHistory[],
    ...
    >({
      ...
      providesTags: (result, error, { projectId }) => [
        { type: 'Performance', id: projectId },
      ],
      // The original slice handled a potential { data: [...] } structure, replicate if necessary
      transformResponse: (
        response: { data: BotPerformanceHistory[] } | BotPerformanceHistory[]
      ) => {
    ...
    getProfitTrending: builder.query<
      TrendDataPoint[],
      { projectId: string; startDate: string; endDate: string }
    >({
      query: ({ projectId, startDate, endDate }) => ({
      ...
      providesTags: (result, error, { projectId }) => [
        { type: 'Trends', id: `${projectId}-profit` },
      ],
    }),
    getVolumeTrending: builder.query<
      TrendDataPoint[],
      { projectId: string; startDate: string; endDate: string }
    >({
      query: ({ projectId, startDate, endDate }) => ({
      ...
      providesTags: (result, error, { projectId }) => [
        { type: 'Trends', id: `${projectId}-volume` },
      ],
    }),
