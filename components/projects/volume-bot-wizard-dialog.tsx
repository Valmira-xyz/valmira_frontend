...
import { updateVolumeGeneration, toggleBot, updateBotConfig } from "@/store/slices/botSlice"
import { projectService } from "@/services/projectService"
import { getWalletBalances } from "@/services/web3Utils"
import { fetchProjects, fetchProject } from "@/store/slices/projectSlice"
...
export function VolumeBotWizardDialog({
  ...
  const loadBotConfiguration = async () => {
    ...
    setIsLoading(true);
    try {
      const botDetails = await BotService.getBotById(project.addons.VolumeBot._id as string);
      ...
      if (botDetails) {
        ...
        const balance = await getWalletBalances([project.addons.VolumeBot.depositWalletId.publicKey], project.tokenAddress);
    ...
    const handleVolumeGenerationUpdate = (data: { 
      ...
      if (data.error) {
        ...
        if (data.generatedVolume > 0) {
          dispatch(updateVolumeGeneration({
            projectId,
            botId: data.botId,
          ...
          if (project?.addons?.VolumeBot?.depositWalletId?.publicKey) {
            getWalletBalances([project.addons.VolumeBot.depositWalletId.publicKey], project.tokenAddress)
              .then(balances => {
  ...
  const ErrorDisplay = () => {
    ...
    const { title, message } = parseErrorMessage(errorState.message, errorState.details);
  ...
  const handleCopyAddress = async () => {
    if (project?.addons?.VolumeBot?.depositWalletId?.publicKey) {
      await navigator.clipboard.writeText(project.addons.VolumeBot.depositWalletId.publicKey);
      toast({
        title: "Success",
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        toast({
          title: "Success",
          description: "Volume Bot configuration applied and started successfully",
        })
        setTimeout(() => {
            fetchAndFillDetailedProejct(projectId)
        }, 2000);
  ...
  return (
    <Dialog open={open} onOpenChange={() => {
        onOpenChange(false); setIsExecuting(false);
      ...
      <DialogContent className="max-w-2xl">
        <Card className="border-none shadow-none">
          <CardHeader className="px-0 pt-0 pb-4">
            ...
            <CardDescription>
              Configure automated trading parameters to generate volume for your token.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="bg-secondary/30 p-4 rounded-lg space-y-4">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                    <div className="text-sm font-mono bg-secondary/20 p-2 rounded">
                        { abbreviateAddress( project?.addons?.VolumeBot?.depositWalletId?.publicKey ) }
                    </div>
                  <Button
                    variant="ghost"
                    size="icon"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm text-muted-foreground">BNB Balance</Label>
                  <p className="text-lg font-semibold">
                    {bnbBalance?.toFixed(6) || '0.000000'} BNB
                  </p>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground">Generated Volume</Label>
                  <p className="text-lg font-semibold">
                    ${project?.addons?.VolumeBot?.generatedVolume?.toFixed(4) || '0.0000'}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    value={minBNBAmount}
                    onChange={(e) => setMinBNBAmount(parseFloat(e.target.value) || 0.0001)}
                    min={0.0001}
                    max={1.0}
                    step={0.0001}
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    value={maxBNBAmount}
                    onChange={(e) => setMaxBNBAmount(parseFloat(e.target.value) || 0.0002)}
                    min={0.0002}
                    max={2.0}
                    step={0.0002}
                    className="w-24 h-8 px-2 rounded-md border border-input bg-background text-sm"
            ...
            <div className="pt-4 space-y-2">
              {project?.addons?.VolumeBot?.isEnabled || errorState ? (
                ...
                <Button
                  ...
                  disabled={isExecuting || isLoading || !project?.addons?.VolumeBot?._id}
                >
                  {isLoading ? (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  ) : isExecuting ? (
