...
export function BotFilter({
  ...
  const selectedBotName = selectedBot
    ? bots.find((bot) => bot.id === selectedBot)?.name || 'All Bots'
    : 'All Bots';
            ...
            <CommandGroup>
              ...
              >
                <Check
                  className={cn(
                    'mr-2 h-4 w-4',
                    selectedBot === null ? 'opacity-100' : 'opacity-0'
                  )}
                />
                All Bots
              </CommandItem>
              {filteredBots.map((bot) => (
                ...
                >
                  <Check
                    className={cn(
                      'mr-2 h-4 w-4',
                      selectedBot === bot.id ? 'opacity-100' : 'opacity-0'
                    )}
                  />
                  {bot.name}
                </CommandItem>
