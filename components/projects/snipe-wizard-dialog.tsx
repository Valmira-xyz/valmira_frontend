...
import { Loader2, Copy, ExternalLink, Download, Flame, ArrowRightLeft, RefreshCw, XCircle, Plus, ChevronRight, ChevronLeft, CheckCircle, Info } from "lucide-react"
import { useDispatch, useSelector } from "react-redux"
import { generateWallets, getWalletBalances, deleteMultipleWallets, sellTokens, multiSellTokens, collectBnb } from "@/store/slices/walletSlice"
...
export function SnipeWizardDialog({
  ...
  const [walletCount, setWalletCount] = useState(project?.addons?.SnipeBot?.subWalletIds?.length || 5)
            ...
            {wallets.filter(w => w.role !== 'botmain').length > 0 ? (
              <div className="border rounded-md overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {wallets.filter(w => w.role !== 'botmain').map((wallet) => (
              ...
              <div className="flex flex-col items-center justify-center py-8 bg-muted/20 rounded-md border border-dashed">
                <p className="text-muted-foreground mb-3">No sniping wallets created yet</p>
                <Button 
                  onClick={() => handleGenerateWallets()}
                  disabled={isGenerating || isProjectLoading}
                  size="sm"
                        ...
                        return (
                          <TableRow key={wallet.publicKey}>
                            <TableCell className="font-mono text-xs">
                              Wallet {index + 1} <span className="hidden sm:inline">({wallet.publicKey.slice(0, 4)}...{wallet.publicKey.slice(-4)})</span>
                            </TableCell>
      ...
      <CardContent className="px-0 sm:px-6">
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3">
                    <div className="bg-muted/20 p-3 rounded-md">
                      ...
                      <p className="font-medium">{simulationResult.snipingBnb.toFixed(6)} BNB</p>
                    </div>
                    <div className="bg-muted/20 p-3 rounded-md">
                      <p className="text-xs text-muted-foreground mb-1">Gas Cost:</p>
                      <p className="font-medium">{simulationResult.gasCost?.toFixed(6) || "0.000000"} BNB</p>
                    </div>
                    <div className="bg-muted/20 p-3 rounded-md">
                      <p className="text-xs text-muted-foreground mb-1">Total BNB Required:</p>
                      <p className="font-medium">{simulationResult.totalBnbNeeded.toFixed(6)} BNB</p>
                    </div>
                  </div>

                  {simulationResult.gasDetails && (
                    <div className="border-t pt-3 mt-3">
                      <p className="text-xs font-medium mb-2">Detailed Gas Costs:</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                        {simulationResult.gasDetails.snipeGas && (
                          <div>
                            <p className="text-muted-foreground">Snipe Gas:</p>
                            <p>{simulationResult.gasDetails.snipeGas?.toFixed(6)} BNB</p>
                          </div>
                        )}
                        {simulationResult.gasDetails.distributionGas && (
                          <div>
                            <p className="text-muted-foreground">Distribution Gas:</p>
                            <p>{simulationResult.gasDetails.distributionGas?.toFixed(6)} BNB</p>
                          </div>
                        )}
                        {simulationResult.gasDetails.openTradingGas && (
                          <div>
                            <p className="text-muted-foreground">Open Trading Gas:</p>
                            <p>{simulationResult.gasDetails.openTradingGas?.toFixed(6)} BNB</p>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {!simulationResult.sufficientBalance && (
                    <div className="bg-red-50 border border-red-200 p-3 rounded-md text-red-700 text-sm mt-3">
                      <p className="font-medium mb-1">⚠️ Insufficient BNB Balance</p>
                      <p>Your deposit wallet has {simulationResult.currentBnbBalance.toFixed(6)} BNB but needs {simulationResult.totalBnbNeeded.toFixed(6)} BNB.</p>
                      <p className="mt-1">Please add {(simulationResult.totalBnbNeeded - simulationResult.currentBnbBalance).toFixed(6)} BNB to continue.</p>
                    </div>
                  )}
                </div>
              ) : (
            ...
            <div className="border rounded-lg p-2 sm:p-4">
              <h3 className="text-base font-medium mb-2 sm:mb-3">BNB Distribution</h3>
              <p className="text-sm text-muted-foreground mb-2 sm:mb-4">
              ...
              <div className="bg-muted/20 p-3 rounded-md">
                ...
                {isSimulating ? (
                  ...
                  <div className="text-sm">
                    <p className={simulationResult.sufficientBalance ? "text-green-600" : "text-red-600"}>
                      {simulationResult.sufficientBalance 
            ...
            <div className="border rounded-lg p-2 sm:p-4">
              ...
              <div className="text-sm space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2">
                  <p><span className="text-muted-foreground">Deposit Wallet BNB Balance:</span> {wallets.find(w => w.publicKey === project?.addons.SnipeBot.depositWalletId?.publicKey)?.bnbBalance?.toFixed(4) || '0.0000'} BNB</p>
                  <p><span className="text-muted-foreground">BNB for Adding Liquidity:</span> {simulationResult.addLiquidityBnb.toFixed(4)} BNB</p>
                  <p><span className="text-muted-foreground">BNB for Sniping wallets:</span> {simulationResult.snipingBnb.toFixed(6)} BNB</p>
                  {simulationResult.tipBnb !== undefined && (
                    <p><span className="text-muted-foreground">BNB for Bundle Tip:</span> {simulationResult.tipBnb.toFixed(4)} BNB</p>
                  )}
                  {simulationResult.gasCost !== undefined && (
                    <p><span className="text-muted-foreground">Gas Cost:</span> {simulationResult.gasCost.toFixed(6)} BNB</p>
                  )}
                  <p><span className="text-muted-foreground">Total spending BNB:</span> {simulationResult.totalBnbNeeded.toFixed(6)} BNB</p>
                </div>
                ...
                {simulationResult.poolSimulation && simulationResult.poolSimulation.initialReserves && (
                  <div className="mt-3 p-2 rounded border bg-background">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1">
                      <p><span className="text-muted-foreground">Initial BNB:</span> {simulationResult.poolSimulation.initialReserves.bnb.toFixed(4)} BNB</p>
                      <p><span className="text-muted-foreground">Initial Tokens:</span> {simulationResult.poolSimulation.initialReserves.token.toLocaleString()} Tokens</p>
                      {simulationResult.poolSimulation.finalReserves && (
                        <>
                          <p><span className="text-muted-foreground">Final BNB:</span> {simulationResult.poolSimulation.finalReserves.bnb.toFixed(4)} BNB</p>
                          <p><span className="text-muted-foreground">Final Tokens:</span> {simulationResult.poolSimulation.finalReserves.token.toLocaleString()} Tokens</p>
                        </>
                      )}
                      {simulationResult.poolSimulation.priceImpact !== undefined && (
                        <p><span className="text-muted-foreground">Price Impact:</span> {(simulationResult.poolSimulation.priceImpact * 100).toFixed(2)}%</p>
                      )}
                    </div>
                  </div>
                )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                ...
                <div className="bg-muted/20 p-3 rounded-md">
                  ...
                  <ul className="mt-1 space-y-1 text-sm">
                    ...
                    <li><span className="text-muted-foreground">Total Tokens to Buy:</span> {wallets
                      ...
                      .reduce((sum, wallet) => sum + (wallet.tokenAmount || 0), 0)
                      .toLocaleString()}</li>
                    <li><span className="text-muted-foreground">Total BNB Required:</span> {simulationResult ? simulationResult.totalBnbNeeded.toFixed(6) : '0.000000'} BNB</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="border rounded-lg p-2 sm:p-4">
              ...
              <Button
                ...
                className="w-full mb-4"
                variant={insufficientFundsDetails || !simulationResult?.sufficientBalance ? "outline" : "default"}
              >
                {isExecuting ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ...
              {!insufficientFundsDetails && simulationResult && !simulationResult.sufficientBalance && (
                <div className="bg-red-50 border border-red-200 p-3 rounded-md text-red-700 text-sm">
              ...
              {!insufficientFundsDetails && (!simulationResult || !isBnbDistributed) && (
                  ...
                  <TableBody>
                    {wallets.length > 0 ? (
                      wallets.map((wallet) => (
                        <TableRow key={wallet.publicKey}>
                          <TableCell>
                            <Badge variant={wallet.role === 'botmain' ? 'secondary' : 'outline'} className="text-xs">
  ...
  const handleGenerateWallets = async () => {
    ...
    const existingWalletCount = project?.addons.SnipeBot.subWalletIds?.length || 0;
    ...
    if (requestedCount < 0) {
      const walletsToDelete = project?.addons.SnipeBot.subWalletIds.slice(walletCount).map(w => w._id);
    ...
    try {
      ...
      if (response.success) {
        ...
        const simulationData = {
          ...
          sufficientBalance: data.depositWalletRequirements.currentBnb >= data.totalBnbNeeded,
  ...
  const handleSingleSell = async (walletPublicKey: string, percentage: number) => {
    if (!project || !project.tokenAddress || percentage <= 0 || percentage > 100) {
      toast({
    ...
    try {
      ...
      const response = await dispatch(sellTokens({
        projectId: project._id,
      ...
      setExecutingSingleSells(prev => ({ ...prev, [walletPublicKey]: false }));
    }
  };
  ...
  const handleMultiSell = async () => {
    ...
    try {
      ...
      const response = await dispatch(multiSellTokens({
        projectId: project._id,
        tokenAddress: project.tokenAddress,
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "An error occurred during multi sell",
        variant: "destructive",
      });
    } finally {
  ...
  const handleCollectBnb = async () => {
    ...
    const depositWallet = wallets.find(w => w.role === 'botmain');
    const snipingWallets = wallets.filter(w => 
      w.role !== 'botmain' && 
      (w.bnbBalance || 0) > 0.001 // Only collect from wallets with sufficient BNB
    );
    ...
    try {
      ...
      const response = await dispatch(collectBnb({
        projectId: project._id,
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "An error occurred during BNB collection",
        variant: "destructive",
      });
    } finally {
